...
export function ImportDatasetDialog({
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Import Dataset">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {tab === 'upload' ? (
          <Field label="Dataset source" htmlFor="zip-input">
            <div className="border-2 border-dashed border-border rounded-lg p-4 space-y-3">
              {selectionLabel ? (
                <div className="flex items-center justify-between text-sm">
                  ...
                  <button type="button" onClick={clearSelection} className="ml-2 flex-shrink-0 text-text-muted hover:text-text-primary">✕</button>
                </div>
              ) : (
                <p className="text-sm text-text-muted text-center">Select a zip file or a local folder</p>
              )}
            ...
            <input id="zip-input" ref={zipRef} type="file" accept=".zip" className="hidden"
              onChange={(e) => { setFolderFiles(null); setZipFile(e.target.files?.[0] ?? null) }} />
            {/* ponytail: webkitdirectory not in React types */}
          ...
          <Field label="Server-side folder path" htmlFor="folderPath">
            <Input id="folderPath" placeholder="/data/my-dataset" value={folderPath}
        ...
        <Field label="Label set (ontology)" htmlFor="ontologyId">
          {ontsLoading ? (
            ...
            <Select id="ontologyId" value={ontologyId} onChange={(e) => setOntologyId(e.target.value)}>
        ...
        <fieldset className="space-y-2">
          <legend className="text-sm font-medium text-text-secondary">
            Split <span className="font-normal text-text-muted">(fallback — ignored if dataset has train/val/test folders)</span>
          </legend>
        ...
        <label className="flex items-center gap-2 text-sm text-text-primary cursor-pointer select-none">
          <input type="checkbox" checked={review} onChange={(e) => setReview(e.target.checked)} className="rounded" />
          Send to CVAT for human review before committing
        </label>

        {(zipping || progress !== null) && (
          <div className="h-1.5 bg-surface-3 rounded-full overflow-hidden">
            <div className="h-full bg-iris-400 transition-all"
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={!canSubmit} loading={busy}>{buttonLabel}</Button>
        </div>
      </form>
    </Dialog>
  )
